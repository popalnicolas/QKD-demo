  ...
  const generateArrays = (size: number) => {
    ...
    if(size >= 10 && size <= 150){
      ...
      array2.map((e, i) => {
        ...
        if(bobsResult !== "-" && bobsResult !== "+"){
          if(Number(bobsResult) !== array4[i])
            setDetected(true);
        }
      });
    }
    ...
    <Container sx={{paddingTop: 20, paddingBottom: 20}}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} textAlign="center" marginRight={20} marginLeft={20}>
          ...
          <Typography color={step === 3 ? white : greyLight} sx={{cursor: "pointer"}} onClick={() => setStep(3)}>Eavesdropper .</Typography>
